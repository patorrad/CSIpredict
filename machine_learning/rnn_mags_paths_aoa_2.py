...
for scaler_type in ['minmax', 'quantiletransformer-gaussian', 'quantiletransformer-uniform']: #, 'yeo-johnson',]:
    ...
    batch_size = 10000
    shuffle = True
    input_size = 139 # FREQUENCY + NUM HITS
    hidden_size = 64
    num_layers = 5
    output_size = 128
    ...
    if TENSORBOARD:
        writer = SummaryWriter(f"runs_aoas_25000_paths/weighted_{model_type}_{num_epochs}_{num_layers}_{hidden_size}_{learning_rate}_{dropout}_{NUM_PATHS}_{batch_size}_{SCALER}_{current.month}-{current.day}-{current.hour}:{current.minute}")
