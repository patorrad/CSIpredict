...
# **** Optional, add a point from the dataset (between 0 to 40400) equal to r_pt and run to see a plot of the clustered angles
...
def kmeans_cluster_aoas(data_aoas,converted_aoas_angles,mag_samples):
    """
    First choice was kmeans clustering because it was a common clustering algorithm.
    Here we find an optimal k and use that to cluste the angle of arrivals. 
    Result: not useful in this case because a specific k must be set => needs extra computation. 
    """
    means_aoa = []
    inertias_aoa = []
    optimal_k_aoa = None
